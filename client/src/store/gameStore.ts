...
export const useGameStore = create<GameStore>((set, get) => ({
  ...
  attemptAttack: (attackerId, targetId, useSpecial = false) => {
    try {
      ...
      // Se o alvo morreu, mantemos ele no board temporariamente com HP 0 para a animação
      if (targetDied) {
  ...
  attemptPlayCard: (cardId, targetHex) => {
    try {
      ...
      // Identificar unidades que morfaram/morreram neste frame
      const deadUnitIds = Object.keys(currentGameState.boardUnits).filter(id => !newState.boardUnits[id]);
      
      // Trigger de Animação de Raio (Lógica específica para o visual)
      ...
      if (deadUnitIds.length > 0) {
        deadUnitIds.forEach(id => {
          ...
          // Se não tiver animação customizada (como raio), aplicamos 'damaged' para piscar
  ...
  sandboxPlayCard: (cardId, hex, playerId) => {
    ...
    // Reproduzir lógica de playCard mas SEM checagens de custo/mão
    if (cardId.startsWith('unit_')) {
    ...
    // Para simplificar e manter a consistência com as regras do engine (morte de rei, etc)
    // Vamos apenas DAR MANA INFINITA e COLOCAR NA MÃO temporariamente, depois jogar e limpar.
    ...
    try {
      ...
      // Restaurar mana (opcional, sandbox!), mas garantir que a carta saiu da mão se falhou
      set(state => ({
        players: {
          ...state.players,
          [playerId]: {
