...
export const BattleHUD: React.FC = () => {
  ...
  return (
    <div className="flex flex-col md:flex-row items-center gap-1 md:gap-4 w-full px-2 md:px-4 py-1 md:py-2 bg-slate-950/90 backdrop-blur-md border-b border-slate-800 z-20 shrink-0 relative">
      ...
      <div className="sm:hidden absolute top-full right-2 mt-2 z-30 flex flex-col items-end gap-2">
        <button 
          onClick={toggleLog}
          className={`
        ...
        <AnimatePresence>
          {isLogVisible && (
            <motion.div
              ...
              className="w-[85vw] max-w-xs pointer-events-auto"
            >
              <BattleLog />
            </motion.div>
          )}
      ...
      {isPvP && (
        <div className="fixed bottom-1 left-1/2 -translate-x-1/2 flex items-center gap-4 text-[8px] text-white/30 z-[100] bg-black/60 px-3 py-1 rounded-full backdrop-blur-md border border-white/10">
          ...
          {/* Botão de Contingência (Oculto pois a sincronização estabilizou)
          <button 
            onClick={() => {
