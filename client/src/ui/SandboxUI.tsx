...
import { UNIT_STATS, ARTIFACTS, SPELLS, ARTIFACT_NAMES, SPELL_NAMES } from 'shared';
    ...
    >
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-white/90 font-black tracking-widest text-[11px] uppercase">{t.warSimulator}</h2>
        </div>
      ...
      <div className="flex border-b border-white/5 shrink-0">
        <button onClick={() => setActiveTab('unidades')} className={`flex-1 py-3 text-[9px] font-black uppercase tracking-widest transition-all ${activeTab === 'unidades' ? 'bg-white/10 text-white border-b-2 border-blue-500' : 'text-white/40 hover:bg-white/5'}`}>{t.units}</button>
        <button onClick={() => setActiveTab('cartas')} className={`flex-1 py-3 text-[9px] font-black uppercase tracking-widest transition-all ${activeTab === 'cartas' ? 'bg-white/10 text-white border-b-2 border-blue-500' : 'text-white/40 hover:bg-white/5'}`}>{t.cards}</button>
      </div>
                  ...
                  {SPELLS.map(spell => (
                    <div key={spell.id} className="flex gap-1">
                      <button disabled={!selectedHex} onClick={() => selectedHex && sandboxPlayCard(spell.id, selectedHex, selectedPlayer)} className={`flex-1 p-2 bg-white/5 disabled:opacity-20 border border-white/5 rounded-lg text-left text-[10px] text-white/70 hover:text-white font-medium flex justify-between items-center group ${selectedPlayer === 'p2' ? 'hover:bg-red-600/20' : 'hover:bg-blue-600/20'}`}>
                        <span>{SPELL_NAMES[spell.id]?.[language] || spell.name}</span>
                        <span className={`text-[7px] px-1 py-0.5 rounded uppercase font-black ${selectedPlayer === 'p1' ? 'bg-purple-500/20 text-purple-300' : 'bg-red-500/20 text-red-300'}`}>{t.cast}</span>
                      </button>
                      <button onClick={() => addCardToHand(spell.id)} className="px-2 bg-white/5 hover:bg-white/10 border border-white/5 rounded-lg text-white/30 hover:text-white transition-colors" title={t.toHand}>🎴</button>
                    </div>
                  ))}
                  {ARTIFACTS.map(art => (
                    <div key={art.id} className="flex gap-1">
                      <button disabled={!selectedHex} onClick={() => selectedHex && sandboxPlayCard(art.id, selectedHex, selectedPlayer)} className={`flex-1 p-2 bg-white/5 disabled:opacity-20 border border-white/5 rounded-lg text-left text-[10px] text-white/70 hover:text-white font-medium flex justify-between items-center group ${selectedPlayer === 'p2' ? 'hover:bg-red-600/20' : 'hover:bg-blue-600/20'}`}>
                        <span>{ARTIFACT_NAMES[art.id]?.[language] || art.name}</span>
                        <span className={`text-[7px] px-1 py-0.5 rounded uppercase font-black ${selectedPlayer === 'p1' ? 'bg-blue-500/20 text-blue-300' : 'bg-red-500/20 text-red-300'}`}>{t.equip}</span>
                      </button>
                      <button onClick={() => addCardToHand(art.id)} className="px-2 bg-white/5 hover:bg-white/10 border border-white/5 rounded-lg text-white/30 hover:text-white transition-colors" title={t.toHand}>🎴</button>
                    </div>
                  ))}
             </div>
          </div>
      ...
      <div className="p-3 bg-black/40 border-t border-white/10 shrink-0 relative flex gap-2">
        ...
        {import.meta.env.DEV && (
          ...
          >
            {isAutoPlay ? `🛑 ${t.stopSimulation}` : `🤖 ${t.simulate}`}
          </button>
        )}
        {/* Resize Handle */}
